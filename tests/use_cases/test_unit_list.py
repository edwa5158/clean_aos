...
@pytest.fixture
def domain_units():
    clanrats = Unit(code=uuid.uuid4(), name="Clanrats", is_hero=False)

    grey_seer = Unit(code=uuid.uuid4(), name="Grey Seer", is_hero=True)

    grey_seer_on_screaming_bell = Unit(
        code=uuid.uuid4(), name="Grey Seer on Screaming Bell", is_hero=True
    )

    rat_ogors = Unit(code=uuid.uuid4(), name="Rat Ogors", is_hero=False)
